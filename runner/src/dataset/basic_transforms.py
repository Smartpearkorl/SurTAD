...
from runner.src.dataset.rand_augment import rand_augment_transform
...
def pad_wide_clips(h, w, crop_h, crop_w):
    """
    返回一个 do_pad(x) 函数，对输入的 HxWx3 图像进行纵向 padding，
    并最终 resize 到 (crop_h, crop_w)。

    Args:
        h, w: 输入图片高度和宽度
        crop_h, crop_w: 最终输出图片大小（可以不同）
    """
    ...
    if (padding_mode is not None) and h_to_sq > 0:
        ...
        a = torch.rand(1).item() * 0.7  # for reflect blending

        # 1. reflect padding + blend
        if padding_mode == "reflect":
            def _do_pad(x, alpha):
                ...
                blended = cv2.addWeighted(reflect_padded, alpha, black_padded, 1 - alpha, 0)
                return cv2.resize(blended, dsize=(crop_w, crop_h), interpolation=cv2.INTER_CUBIC)
        ...
        # 2. replicate padding
            ...
            do_pad = lambda x: cv2.resize(
                cv2.copyMakeBorder(x, pad_top, pad_bottom, 0, 0, cv2.BORDER_REPLICATE),
                dsize=(crop_w, crop_h), interpolation=cv2.INTER_CUBIC
            )

        # 3. black / color padding
        elif padding_mode in ('black', 'color'):
            color = torch.randint(0, 256, (3,)).tolist() if padding_mode == 'color' else [0, 0, 0]
        ...
        # 不进行实际 padding，直接 resize 到 crop_h, crop_w
        do_pad = lambda x: cv2.resize(x, dsize=(crop_w, crop_h), interpolation=cv2.INTER_CUBIC)
