'''
Credit to the official implementation:
...
def register_vst_model(type, checkpoint=None):
    from collections import OrderedDict
    if type == 'swin_base_patch244_window1677_sthv2':
        mod_kwargs = {
            'depths': [2, 2, 18, 2],
            'embed_dim': 128,
            'num_heads': [4, 8, 16, 32],
        ...
        vst = SwinTransformer3D(**mod_kwargs)
    elif type == 'swin_base_patch244_window877_kinetics400_22k':
        mod_kwargs = {
            'depths': [2, 2, 18, 2],        # Swin-B 标准深度
            'embed_dim': 128,               # 基础嵌入维度
            'num_heads': [4, 8, 16, 32],    # 四个 stage 的 multi-head attention heads
            'drop_path_rate': 0.5,          # K400-22k 预训练通常更高（0.5）
            'patch_size': (2, 4, 4),        # Swin video 的标准时空 patch
            'window_size': (8, 7, 7),       # ★ 与 sth-v2 版本区别：时间维 window 为 8 而不是 16
            'patch_norm': True,
            'in_chans': 3,
        }
        vst = SwinTransformer3D(**mod_kwargs)
    ...
    if checkpoint is not None:
        ...
        vst.load_state_dict(new_state_dict,strict=True)
    return vst
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.use_checkpoint:
            x = x + \
                (checkpoint.checkpoint(self.forward_part2, x))
        else:
