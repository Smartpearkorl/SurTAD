    ...
    def __call__(self, img):
        for t in self.transforms:
            if isinstance(t, Sequence):
                assert isinstance(img, Sequence) and len(img) == len(
...
class Merge(object):
    ...
    def __init__(self, axis=-1):
        self.axis = axis
    def __call__(self, images):
        if isinstance(images, Sequence) or isinstance(images, np.ndarray):
...
class ArrayToTensorNumpy(object):
    ...
    def __call__(self, images):
        ...
        if isinstance(images, list):
            ...
            im = images.transpose((2, 0, 1))
            # handle numpy array
            tensors = torch.from_numpy(im)
        return tensors
...
class Split(object):
    ...
    def __init__(self, *slices, **kwargs):
        assert isinstance(slices, Sequence)
        slices_ = []
        for s in slices:
